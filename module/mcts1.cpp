...
static void init_chess(chess x);//初始化棋盘状态 
static void init_window();//初始化窗口
static void game_window();//游戏窗口 
static void print_board(chess x);//打印棋盘 
static void set_pos(int x,int y);//调整光标位置 
static void white_win();//白棋胜利 
static void black_win();//黑棋胜利 
...
void print_board(chess x){
	...
	for(int i=0;i<=row;i++)
	{
		...
		for(int j=0;j<=col;j++){
			if(x.g[i][j]==0)p=' ';
			else if(x.g[i][j]==1)p='1';
			else if(x.g[i][j]==2)p='2';
			cout<<"|"<<p<<" ";
		}
		cout<<"|";
		cout<<"\n";
...
chess UCT_search(chess x,pair<int,int> center,int player){//x
	chess y=x;
	chess ans=y;
